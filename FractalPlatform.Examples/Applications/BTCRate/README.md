# BTC Rate

## Sources

- [Application](https://github.com/fraplat/FractalPlatform/tree/main/FractalPlatform.Examples/Applications/BTCRate/BTCRateApplication.cs)

## Functionality

Application shows web form based on REST api call.
In current case application shows BTC price in USD/EUR currencies from cryptocompare.com market.

## How it implemented

**Video explanation**

- [EN UTube Video](https://fraplat.dev/jupiter/UTube?tag=110)
- [UA UTube Video](https://fraplat.dev/jupiter/UTube?tag=210)

## Web Link

[BTCRate](https://fraplat.dev/jupiter/BTCRate)

## Open app in Fractal Studio

[Play with BTCRate in sandbox](https://fraplat.dev/mars/FractalStudio/?tag=BTCRate+template)


