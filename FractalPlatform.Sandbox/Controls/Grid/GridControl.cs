...
namespace FractalPlatform.Sandbox.Controls.Grid
{
	public class GridControl : BaseControl
	{
		...
		{
			var contextMenus = ((GridDOMControl)DOMControl).ContextMenus;

			if (contextMenus != null && contextMenus.Count > 0)
			{
				_lvGrid.MouseDown += _lvGrid_MouseDown;
			}
		}

		private void _lvGrid_MouseDown(object sender, MouseEventArgs e)
		{
			if (e.Button != MouseButtons.Right)
				return;

			var domControl = (GridDOMControl)DOMControl;
			var contextMenus = domControl.ContextMenus;

			var hitInfo = _lvGrid.HitTest(e.Location);
			if (hitInfo.Item == null) return;

			int itemIndex = hitInfo.Item.Index;

			if (itemIndex < 0 || itemIndex >= contextMenus.Count)
				return;

			var menuItems = contextMenus[itemIndex];
